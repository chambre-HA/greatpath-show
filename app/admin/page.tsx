...
import type { ClassInfo, DingkeSection, School as SchoolInfo, DingkeVariant, ShowLink } from '@/types'
...
function TagBadges({ tags }: { tags?: string[] }) {
  ...
  return (
    <span className="flex flex-wrap gap-1">
      {tags.map(t => (
        <span key={t} className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-slate-800 text-slate-300 whitespace-nowrap">
          {t}
        </span>
      ))}
    </span>
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      <button
        ...
        className={`px-2.5 py-1 rounded-full text-[11px] font-bold smooth-transition ${
          active === null ? 'bg-emerald-600 text-white' : 'bg-slate-900 text-slate-400 hover:text-slate-200 border border-slate-800'
        }`}
      >
        全部
      </button>
      {tags.map(t => (
        <button
          ...
          onClick={() => onSelect(t)}
          className={`px-2.5 py-1 rounded-full text-[11px] font-bold smooth-transition ${
            active === t ? 'bg-emerald-600 text-white' : 'bg-slate-900 text-slate-400 hover:text-slate-200 border border-slate-800'
          }`}
        >
          {t}
        </button>
  ...
  return (
    <div ref={rootRef} className="relative">
      <div
        ...
        className="w-full min-h-[2.5rem] px-2.5 py-1.5 rounded-xl bg-slate-950 border border-slate-800 focus-within:border-emerald-500 flex flex-wrap gap-1.5 items-center cursor-text transition-all"
      >
        {value.map(tag => (
          <span key={tag} className="flex items-center gap-1 pl-2 pr-1 py-0.5 rounded-full text-[11px] font-bold bg-slate-800 text-slate-200 shrink-0">
            {tag}
            <button
              type="button"
              onClick={e => { e.stopPropagation(); removeTag(tag) }}
              className="p-0.5 rounded-full hover:bg-slate-700 text-slate-400 hover:text-white smooth-transition"
              aria-label={`移除标签 ${tag}`}
            >
              <X size={10} />
            </button>
        ...
        <input
          ...
          className="flex-1 min-w-[6rem] bg-transparent text-sm text-slate-100 placeholder-slate-600 focus:outline-none"
        />
      </div>
      {open && (filtered.length > 0 || (q.length > 0 && !exactExists)) && (
        <div className="absolute left-0 right-0 top-full mt-1 rounded-xl border border-slate-800 bg-slate-950 shadow-2xl z-50 max-h-48 overflow-y-auto py-1">
          {q.length > 0 && !exactExists && (
            <button
              type="button"
              onClick={() => addTag(query)}
              className="w-full px-3 py-2 text-left text-xs font-bold text-emerald-400 hover:bg-slate-900 smooth-transition flex items-center gap-1.5"
            >
          ...
          {filtered.map(tag => (
            <button
              ...
              type="button"
              onClick={() => addTag(tag)}
              className="w-full px-3 py-2 text-left text-xs font-semibold text-slate-300 hover:bg-slate-900 hover:text-white smooth-transition"
            >
              {tag}
            </button>
          ))}
                        ...
                        <>
                          <div className="flex items-start gap-3">
                            <button
                              ...
                              className="shrink-0 mt-1.5 p-1 -ml-1 rounded text-slate-650 hover:text-slate-300 cursor-grab active:cursor-grabbing touch-none disabled:opacity-20 disabled:pointer-events-none smooth-transition"
                              aria-label="拖拽排序"
                            >
                            ...
                            <div className="flex-1 min-w-0">
                              {isEditing ? (
                                ...
                                <div className="pt-0.5 space-y-1.5">
                                  ...
                                  <p className="text-[10px] text-slate-500 font-mono tracking-wider flex items-center gap-1">
...
function DingkeVariantSectionEditor({
  ...
  return (
    <div className="absolute inset-0 z-50 bg-gray-950/80 backdrop-blur-sm flex items-stretch justify-end">
      <div className="w-full max-w-3xl h-full bg-gray-950 border-l border-gray-800 flex flex-col">
        <header className="shrink-0 px-5 py-4 border-b border-gray-800/80 flex items-center gap-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-slate-500 mt-0.5 font-mono">id: {section.id}</p>
          </div>
        ...
        <div className="flex-1 min-h-0 flex">
          ...
          <div className="hidden md:flex w-[22rem] shrink-0 border-r border-gray-800/80 flex-col gap-3 p-5 bg-black/10 overflow-y-auto">
            <p className="text-[10px] uppercase tracking-wider font-bold text-slate-500">大字页面预览</p>
            <div
              className="rounded-xl overflow-hidden border border-slate-800 shrink-0"
...
function DingkeVariantsManager({
  ...
  const field = 'w-full px-3 py-2 text-xs rounded-xl bg-slate-950 border border-slate-800 text-slate-100 placeholder-slate-650 focus:outline-none focus:border-emerald-500'
      ...
      <div className="max-w-2xl space-y-6">
        ...
        {!isDefaultView && (
          <div className="space-y-3">
            <div>
              <label className="block text-[10px] uppercase tracking-wider font-bold text-slate-500 mb-1.5">版本名称</label>
              <input
                className={field}
                placeholder="如：进阶班定课"
                value={name}
            ...
            {editingId === 'new' && (
              <div>
                <label className="block text-[10px] uppercase tracking-wider font-bold text-slate-500 mb-1.5">起始内容</label>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <p className="text-[10px] uppercase font-bold tracking-wider text-slate-500">环节（{sections.length}）</p>
            {!isDefaultView && (
              <button
                type="button"
                onClick={addSection}
                className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-bold text-emerald-400 hover:bg-emerald-950/30 smooth-transition"
              >
          ...
          {sections.length === 0 && (
            <p className="px-3 py-6 text-xs text-slate-500 italic text-center border border-dashed border-slate-800 rounded-xl">
              ...
              <ul className="space-y-1.5">
                {sections.map((section, i) => (
                  <SortableLinkItem
                    ...
                    className="flex items-center gap-2 p-2.5 rounded-xl border border-slate-800 bg-slate-950/40"
                  >
                    {({ attributes, listeners }) => (
                      <>
                        {!isDefaultView && (
                          <button
                            ...
                            {...attributes}
                            {...listeners}
                            className="shrink-0 p-1 -ml-1 rounded text-slate-650 hover:text-slate-300 cursor-grab active:cursor-grabbing touch-none smooth-transition"
                            aria-label="拖拽排序"
                          >
                            <GripVertical size={14} />
                          </button>
                        )}
                        <span className="text-[10px] font-mono text-slate-600 w-5 text-center shrink-0">{i + 1}</span>
                        <button
                          type="button"
                        ...
                        {!isDefaultView && (
                          <div className="flex items-center gap-0.5 shrink-0">
                            <button type="button" onClick={() => setEditingSectionIndex(i)} className="p-1.5 rounded-lg text-slate-500 hover:text-white hover:bg-slate-900" aria-label="编辑">
                              <Pencil size={13} />
                            </button>
                            <button type="button" onClick={() => removeSection(i)} className="p-1.5 rounded-lg text-slate-500 hover:text-rose-500 hover:bg-rose-950/20" aria-label="删除">
                              <Trash2 size={13} />
                            </button>
                          </div>
                        )}
        ...
        <div className="flex gap-2 pt-2">
          {isDefaultView ? (
            <button
              ...
              className="flex items-center justify-center gap-1.5 px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold active:scale-[0.98] transition-all"
            >
              <Copy size={13} />
              <span>复制为新版本</span>
            </button>
            ...
            <button
              ...
              className="flex items-center justify-center gap-1.5 px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold disabled:opacity-40 active:scale-[0.98] transition-all"
            >
              <Save size={13} />
          ...
          <button
            type="button"
            onClick={resetForm}
            className="px-4 py-2.5 text-xs font-bold rounded-xl border border-slate-800 text-slate-400 hover:text-white hover:bg-slate-900"
          >
            {isDefaultView ? '返回' : '取消'}
          </button>
        </div>
    ...
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center justify-between border-b border-slate-800/60 pb-4">
        ...
        <button
          type="button"
          onClick={startNew}
          className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold active:scale-[0.98] smooth-transition"
        >
      ...
      <ul className="space-y-2">
        ...
        {(() => {
          ...
          return (
            <li className="p-4 rounded-2xl border border-slate-800 bg-slate-950/40 space-y-2">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-sm font-bold text-white flex items-center gap-1.5">
                    <span>默认脚本</span>
                    <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-slate-800 text-slate-300 font-bold shrink-0">内置</span>
                  </p>
                  <p className="text-[10px] text-slate-500 mt-0.5">
        ...
        {variants.length === 0 && (
          <li className="px-3 py-6 text-xs text-slate-500 italic text-center">暂无自定义版本，其余班级使用默认脚本。</li>
        )}
        {variants.map(variant => {
          const assigned = classes.filter(c => c.dingkeVariantId === variant.id)
          return (
            <li key={variant.id} className="p-4 rounded-2xl border border-slate-800 bg-slate-950/40 space-y-2">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
            ...
            return (
              <ul className="space-y-1.5">
                {visible.map(item => {
                  ...
                  <li
                    key={item.url}
                    className={`p-3 rounded-2xl border smooth-transition space-y-2.5 ${
                      isEditing ? 'border-emerald-500/40 bg-slate-900/50' : 'border-slate-850 bg-slate-900/40 hover:border-slate-800'
                    }`}
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0 space-y-1">
                          <p className="text-sm font-medium text-slate-200 truncate max-w-[320px] flex items-center gap-1.5">
                            <span className="truncate">{item.title}</span>
                            {allHidden && (
                              <span className="shrink-0 px-1.5 py-0.5 rounded text-[9px] font-bold bg-slate-800 text-slate-400">全部隐藏</span>
                            )}
                          </p>
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <button
                          ...
                          className="p-2 rounded-xl text-slate-500 hover:text-emerald-400 hover:bg-emerald-950/20 disabled:opacity-30 smooth-transition"
                          title="编辑标签"
                        >
                          <Tag size={14} />
                        </button>
                        <button
                          ...
                          disabled={busyUrl === item.url}
                          className="p-2 rounded-xl text-slate-500 hover:text-rose-500 hover:bg-rose-950/20 disabled:opacity-30 smooth-transition"
                          title="从共享库彻底删除"
                        >
                          <Trash2 size={14} />
                        </button>
                      ...
                      <div className="space-y-2 border-t border-slate-800/40 pt-2.5">
                        ...
                        <div className="flex justify-end gap-3">
                          ...
                          <button
                            ...
                            disabled={busyUrl === item.url}
                            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold disabled:opacity-40 smooth-transition"
                          >
                            <Save size={12} />
...
export default function AdminPage() {
  ...
  // The schools/variants managers show class assignments, and the class panel
  // shows the assigned school/variant, so everything is refreshed together
  // after any change.
  const reloadAll = useCallback(async () => {
    await Promise.all([loadClasses(password), loadSchools(password), loadDingkeVariants(password)])
  }, [loadClasses, loadSchools, loadDingkeVariants, password])

  // Assigning is an update on the class record, not on the school/variant.
  // Both fields are always sent together (full overwrite, not a merge — see
  // updateClass on the server) so assigning one never silently clears the other.
  ...
  return (
    <main className="h-screen bg-slate-955 text-slate-100 premium-glow-bg flex flex-col overflow-hidden">
      {/* Header */}
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <aside className="w-80 shrink-0 border-r border-slate-800/80 flex flex-col bg-slate-950/40">
          ...
          <div className="p-2.5 border-b border-slate-800/60 bg-slate-950/20">
            ...
            <button
              type="button"
              onClick={() => { setShowLibrary(true); setShowSchools(false); setShowDingkeVariants(false); setSelectedCode(null) }}
            ...
            <button
              type="button"
              onClick={() => { setShowSchools(true); setShowLibrary(false); setShowDingkeVariants(false); setSelectedCode(null) }}
            ...
            <button
              type="button"
              onClick={() => { setShowDingkeVariants(true); setShowLibrary(false); setShowSchools(false); setSelectedCode(null) }}
              className={`w-full flex items-center gap-2.5 px-3 py-3 mt-1 rounded-xl border smooth-transition ${
                showDingkeVariants
                  ? 'bg-emerald-600/15 border-emerald-500/30 text-white'
                  : 'border-transparent text-slate-350 hover:bg-slate-900/50 hover:text-white'
              ...
              {classes.map(cls => (
                <li key={cls.code}>
                  <div
                    ...
                    onClick={() => { setSelectedCode(cls.code); setShowLibrary(false); setShowSchools(false); setShowDingkeVariants(false) }}
                  >
                    <div className="flex-1 min-w-0">
            ...
            <div className="max-w-xl space-y-6">
              ...
              <div className="p-4 rounded-2xl border border-slate-800 bg-slate-950/40 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-[10px] uppercase font-bold tracking-wider text-slate-500">定课版本</p>
                </div>
                <select
