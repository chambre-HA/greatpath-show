      ...
      <div className="w-full max-w-md space-y-8 z-10">
        ...
        <div className="text-center space-y-3">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-zinc-400">输入 8 位班级码开启共修之旅</p>
          </div>
        </div>

        {/* Code entry — no card boundary, sits flush on the page */}
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-1.5 sm:gap-2">
              {digits.map((digit, i) => (
                <input
                  ...
                  className={`w-full min-w-0 aspect-square text-center text-2xl font-bold font-mono rounded-[var(--radius-sm)] bg-zinc-800/70 border text-white placeholder-zinc-600 focus:outline-none focus:border-orange-500 transition-all smooth-transition ${
                    error ? 'border-rose-500/60' : 'border-transparent'
                  }`}
                />
              ))}
            </div>
          ...
          {error && (
            <p className="text-xs text-rose-400 text-center bg-rose-950/20 py-1.5 rounded-[var(--radius-sm)] border border-rose-900/30">
              {error}
            </p>
          )}
          ...
          <button
            ...
            className="w-full py-3.5 rounded-[var(--radius-sm)] bg-orange-600 hover:bg-orange-500 text-white font-semibold active:scale-[0.98] disabled:opacity-40 disabled:pointer-events-none transition-all duration-300 flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
            ...
            <div className="space-y-2">
              {recentClasses.map(cls => (
                <div
                  ...
                  className="flex items-center justify-between p-3.5 rounded-[var(--radius-sm)] bg-zinc-800/50 hover:bg-zinc-800 smooth-transition cursor-pointer group/item"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-8 h-8 rounded-[var(--radius-sm)] bg-zinc-700/60 flex items-center justify-center shrink-0">
                      <School size={14} className="text-zinc-400" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-zinc-200 truncate group-hover/item:text-orange-300 transition-colors">
                        {cls.name}
                      </p>
                      <p className="text-[10px] text-zinc-500 font-mono tracking-wider">{cls.code}</p>
                    </div>
                  </div>
                  <button
                    onClick={e => removeFromHistory(e, cls.code)}
                    className="p-1.5 rounded-[var(--radius-sm)] text-zinc-600 hover:text-zinc-300 hover:bg-zinc-700 smooth-transition"
                    title="清除记录"
        ...
        <p className="text-center">
          <a
            href="/admin"
            className="inline-flex items-center gap-1.5 text-xs text-zinc-500 hover:text-zinc-300 smooth-transition"
          >
            <Settings size={12} />
            <span>管理后台</span>
          </a>
