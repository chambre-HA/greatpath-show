...
export function Viewer({ classCode, links, openTabIds, activeTabId, onToggleSidebar, onActivate, onClose, onAdd, onRemove, onRefresh }: ViewerProps) {
  ...
  return (
    <div className="flex-1 flex flex-col bg-zinc-900 min-w-0 overflow-hidden">
      {/* Tab bar */}
      <div className="flex items-stretch bg-zinc-950 border-b border-zinc-800/80 shrink-0">
        <button
          onClick={onToggleSidebar}
          className="p-3 text-zinc-400 hover:text-white [@media(min-width:768px)_and_(min-height:640px)]:hidden border-r border-zinc-800 shrink-0"
          aria-label="Open sidebar"
        >
          <Menu size={18} />
        </button>

        <div ref={filesRef} className="relative shrink-0 border-r border-zinc-800">
          <button
            onClick={toggleFilesOpen}
            className={`flex items-center gap-1.5 px-3 h-full text-sm font-semibold smooth-transition ${
              filesOpen ? 'text-white bg-zinc-900/60' : 'text-zinc-200 hover:text-white hover:bg-zinc-900/60'
            }`}
          >
            <span>选择文件</span>
          ...
          {filesOpen && (
            <div className="absolute left-0 top-full mt-1 w-72 rounded-[var(--radius-sm)] border border-zinc-800 bg-zinc-950 shadow-2xl z-50 flex flex-col max-h-[28rem]">
              <div className="flex-1 min-h-0 overflow-y-auto py-1.5">
                {links.length === 0 ? (
                  <p className="px-4 py-6 text-xs text-zinc-500 italic text-center">暂无文件，点击右侧「添加文件」。</p>
                ) : (
                  groupLinksByTag(links).map(group => (
                    <div key={group.tag ?? '__untagged'}>
                      {group.tag && (
                        <p className="px-4 pt-2.5 pb-1 text-[10px] uppercase font-bold tracking-wider text-zinc-600">
                          {group.tag}
                        </p>
                      )}
                      {group.items.map(link => {
                        ...
                        return (
                          <div
                            ...
                            className={`group flex items-center gap-2.5 px-3 py-2 mx-1.5 rounded-[var(--radius-sm)] cursor-pointer smooth-transition ${
                              checked ? 'bg-orange-600/15 text-white' : active ? 'text-orange-400' : 'text-zinc-300 hover:bg-zinc-900/70 hover:text-white'
                            }`}
                          >
                            <span className={`shrink-0 w-4 h-4 rounded border flex items-center justify-center smooth-transition ${
                              checked ? 'bg-orange-600 border-orange-600' : 'border-zinc-700'
                            }`}>
                            ...
                            <button
                              ...
                              className="opacity-0 group-hover:opacity-100 p-1 rounded text-zinc-500 hover:text-rose-400 hover:bg-zinc-900/80 smooth-transition shrink-0"
                              aria-label="Remove"
                            >
                              <Trash2 size={12} />
                            </button>
              ...
              {links.length > 0 && (
                <div className="shrink-0 border-t border-zinc-800 p-2">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-1.5 py-2 rounded-[var(--radius-sm)] bg-orange-600 hover:bg-orange-500 text-white text-xs font-semibold disabled:opacity-30 disabled:pointer-events-none smooth-transition"
                  >
        ...
        <button
          ...
          className="flex items-center px-2.5 text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900/60 border-r border-zinc-800 shrink-0 smooth-transition"
          title="添加文件"
          aria-label="添加文件"
        >
          <Plus size={14} />
          ...
          {openTabIds.map(id => {
            ...
            return (
              <button
                ...
                className={`group flex items-center gap-2 px-4 py-3 border-r border-zinc-800 shrink-0 max-w-[180px] min-w-0 transition-all smooth-transition ${
                  active
                    ? 'bg-zinc-900 text-white border-b-2 border-b-emerald-500 -mb-px font-semibold'
              ...
              >
                ...
                <span
                  ...
                  className="opacity-100 md:opacity-0 md:group-hover:opacity-100 p-1 rounded hover:bg-zinc-800 text-zinc-500 hover:text-zinc-300 shrink-0 ml-1.5 smooth-transition"
      ...
      {openTabIds.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center px-6">
          <div className="w-14 h-14 rounded-[var(--radius-md)] bg-amber-950/40 border border-amber-900/30 flex items-center justify-center">
            <Presentation size={24} className="text-amber-500" />
          </div>
          <div className="space-y-1.5">
            <h2 className="text-base font-semibold text-white">选择文件开始演示</h2>
            <p className="text-xs text-zinc-500">从已上传的文档中选择，也可添加新文件</p>
          </div>
          <div className="flex items-center gap-2">
            {links.length > 0 && (
              <button
                onClick={() => setFilesOpen(true)}
                className="flex items-center gap-1.5 px-4 py-2 rounded-[var(--radius-sm)] bg-orange-600 hover:bg-orange-500 text-white text-xs font-semibold active:scale-[0.98] transition-all duration-200"
              >
            ...
            <button
              onClick={() => setAdding(true)}
              className="flex items-center gap-1.5 px-4 py-2 rounded-[var(--radius-sm)] border border-zinc-700 text-zinc-400 text-xs font-medium hover:bg-zinc-900 hover:text-zinc-200 active:scale-[0.98] transition-all duration-200"
            >
      ...
      {adding && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm transition-opacity duration-300">
          <div className="w-full max-w-md rounded-[var(--radius-md)] bg-zinc-900 shadow-2xl border border-zinc-800 overflow-hidden flex flex-col scale-100 transition-transform duration-300">
            <div className="px-5 py-4 border-b border-zinc-800 flex items-center justify-between bg-zinc-950/60">
              ...
              <button
                ...
                className="p-1.5 rounded-[var(--radius-sm)] text-zinc-400 hover:text-white hover:bg-zinc-900 smooth-transition"
...
function PptxViewer({ link }: { link: ShowLink }) {
      ...
      <div className="w-full h-full flex items-center justify-center p-8">
        <div className="max-w-lg text-center space-y-4">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-4 py-2 rounded bg-zinc-100 text-zinc-900 text-sm font-medium hover:bg-white"
          >
    ...
    return (
      <div className="w-full h-full flex items-center justify-center p-8">
        <div className="max-w-lg text-center space-y-4">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed">{strategy.reason}</p>
          <div className="text-left text-xs text-zinc-500 bg-zinc-950 border border-zinc-800 rounded-[var(--radius-sm)] p-4 space-y-2">
            <p className="font-semibold text-zinc-300">How to get an embeddable URL:</p>
            <ol className="list-decimal list-inside space-y-1">
              <li>In OneDrive, open the file</li>
              <li>Click <span className="text-zinc-300">Share → Embed</span></li>
              <li>Copy only the <code className="text-zinc-300">src=&quot;…&quot;</code> URL from the iframe snippet</li>
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full bg-zinc-900">
      {!loaded && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 text-zinc-400 z-10 bg-zinc-900">
      ...
      <button
        ...
        className="absolute bottom-4 right-4 p-2 rounded-[var(--radius-sm)] bg-zinc-950/90 backdrop-blur border border-zinc-800 text-zinc-300 hover:text-white"
